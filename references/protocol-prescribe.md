...
**단위 정의:**
- **턴**: Claude 대화 턴 수 (1턴 = 편집 1회 + 검증 1회 묶음)
- **롤백(R)**: L=단순 revert, M=복수 파일 복원, H=의존 스킬 연쇄 복원
- **회귀리스크(RegR)**: L=독립, M=스킬 내부, H=cross-skill cascade
...
- **예상 크기:** 4.8KB (목표 달성)
- **예상 턴:** 2턴 (분리 1 + 검증 1)
- **롤백:** L — 원본 SKILL.md를 `_archive/`에서 복원
- **회귀리스크:** M — 본문 포인터 경로 불일치 시 로딩 실패
...
3. **LLM 비용 추정:** 각 액션에 `턴 수 + 롤백(L/M/H) + 회귀리스크(L/M/H)` 3축 병기. 분·시간 단위 금지 (인간 공수는 LLM 실행 비용과 무관).
4. **롤백 경로 구체화:** 각 액션에 "실패 시 복원 방법 + 원본 위치" 명시.
...
| 축 | 값 | 판정 기준 |
|----|-----|----------|
| **턴 수** | 1턴 | 단일 필드(trigger·frontmatter) 수정 |
|  | 2~3턴 | 섹션 수정 + 검증 |
|  | 3~5턴 | 구조 변경(허브스포크 분리) + 검증 + 재점검 |
|  | 5턴+ | 재설계 — 반드시 사용자 컨펌 요청 |
| **롤백(R)** | L | 원본 1파일 revert |
|  | M | 복수 파일 + 경로 포인터 복원 |
|  | H | 의존 스킬 연쇄 복원 + UP 체크리스트 갱신 |
| **회귀리스크(RegR)** | L | 독립 변경, cascade 없음 |
|  | M | 스킬 내부 상호작용(허브↔스포크) |
|  | H | cross-skill 파급 (UP·다른 스킬 cascade) |

**게이트:** 회귀리스크 H → skill-builder에 "중간" 이상 경로 강제 + Before/After 컨펌.
...
- **분·시간 단위 추정 = FAIL.** LLM 실행에는 턴 수만 유효. man-hour 사용 시 사용자 기대치 왜곡
- **턴·롤백·회귀리스크 3축 누락** = 사용자 판단 불가. 반드시 병기
